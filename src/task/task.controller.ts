...
export class TaskController {
  ...
  @ApiResponse({
    description: 'Tasks could not be retrieved. Please try again later.',
    status: 500,
  })
  @ApiResponse({
  ...
  getTasks(@User() user, @Query() query_params) {
    if (query_params.due_date) {
      const due_date = moment(query_params.due_date).format(
  ...
  @Post('create')
  createTask(@User() user, @Body() dto: TaskDto) {
